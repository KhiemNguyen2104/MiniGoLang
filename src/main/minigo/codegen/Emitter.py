...
class Emitter():
    ...
    def getJVMType(self, inType):
        ...
        if typeIn is IntType:
            ...
            s = ""
            for i in range(len(inType.dimens)):
                s += "["
            return s + self.getJVMType(inType.eleType)
    ...
    def emitMULTIANEWARRAY(self, typ, dimens, frame):
        s = ""
        for _ in range(len(dimens)):
            frame.pop()
            s += "["
        s += self.getJVMType(typ)

        frame.push()

        return self.jvm.emitMULTIANEWARRAY(s, str(len(dimens)))
    ...
    def emitSTRINGLIT(self, in_, frame):
        frame.push()
        return self.jvm.emitLDC('"' + in_ + '"')
    ...
    *
    ...
    def emitWRITEVAR2(self, name, typ, frame):
        ...
        if type(typ) is IntType:
            ...
            return self.jvm.emitFASTORE()
        elif type(typ) is cgen.ArrayType or type(typ) is StringType or type(typ) is cgen.ClassType:
            return self.jvm.emitAASTORE()
        else:
            raise IllegalOperandException(name)
    ...
    *   @param isFinal true in case of constant; false otherwise
    '''
    def emitATTRIBUTE(self, lexeme, in_, isStatic, isFinal, value = None):
        #lexeme: String
        #in_: Type
        #isFinal: Boolean
        #value: String
    ...
    *   generate ior
    ...
    def emitREOP(self, op, in_, frame):
        ...
        if type(in_) is IntType:
            ...
            if op == ">":
                result.append(self.jvm.emitINVOKEVIRTUAL("java/lang/String/compareTo", self.getJVMType(cgen.MType([StringType()], IntType()))))
                result.append(self.jvm.emitIFLE(labelF))
            elif op == ">=":
                result.append(self.jvm.emitINVOKEVIRTUAL("java/lang/String/compareTo", self.getJVMType(cgen.MType([StringType()], IntType()))))
                result.append(self.jvm.emitIFLT(labelF))
    ...
    def emitSHORTCIRCUIT(self, op, in_, lCode, rCode, lType, rType, trueLabel, falseLabel, frame):
        result = list()
        
        frame.pop()
        frame.pop()
        if op == '&&':
            if type(lType) is BinaryOp:
                result.append(lCode[0])
                result.append(lCode[1])
                op = lType.op
                if type(lType.left) is IntType:
                    if op == ">":
                    ...
                    if type(rType) is BinaryOp:
                        result.append(rCode[0])
                        result.append(rCode[1])
                        rOp = rType.op
                        if type(rType.left) is IntType:
                            if rOp == ">":
                                result.append(self.jvm.emitIFICMPLE(falseLabel))
                            elif rOp == ">=":
                                result.append(self.jvm.emitIFICMPLT(falseLabel))
                                ...
                                result.append(self.jvm.emitIFICMPNE(falseLabel))
                        elif type(rType.left) is FloatType:
                            if rOp == ">":
                                result.append(self.jvm.emitFCMPL())
                        ...
                        result.append(rCode[0])
                        result.append(self.jvm.emitIFEQ(falseLabel))
                elif type(lType.left) is FloatType:
                    if op == ">":
                        result.append(self.jvm.emitFCMPL())
                        result.append(self.jvm.emitIFLE(falseLabel))
                    elif op == ">=":
                    ...
                    if type(rType) is BinaryOp:
                        ...
                        result.append(rCode[0])
                        result.append(self.jvm.emitIFEQ(falseLabel))
                elif type(lType.left) is StringType:
                    if op == ">":
                        result.append(self.jvm.emitINVOKEVIRTUAL("java/lang/String/compareTo", self.getJVMType(cgen.MType([StringType()], IntType()))))
                        result.append(self.jvm.emitIFLE(falseLabel))
                    elif op == ">=":
    ...
    *   @param in the type of the returned expression.
    ...
    def emitRETURN(self, in_, frame):
        ...
        elif type(in_) is ArrayType or type(in_) is StringType or type(in_) is StructType or type(in_) is InterfaceType:
            frame.pop()
